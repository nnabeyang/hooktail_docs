...
class ReSTWriter(writers.Writer):
    supported = ('text',)
    ...
    def translate(self):
        visitor = ReSTTranslator(self.document, self.builder)
        self.document.walkabout(visitor)
        self.output = visitor.body
	self.links = visitor.links.values()

class ReSTTranslator(nodes.NodeVisitor):
    sectionchars = '*=-~"+`'
	...
	if self.sectionlevel == 1:
          self.states[-1].append(
                (0, ['%s' % ('=' * column_width(text)), text, '%s' % ('=' * column_width(text)), '']))
        else:
          self.states[-1].append(
	...
	img = node.children.pop(0)
	#print img
	#print self.builder.images
	self.add_text(_('.. figure:: %s\n%s') % (img['uri'].split('/')[-1], os.linesep))
	...
	#keys = ['alt', 'width', 'height', 'scale', 'align']
	buf = []
	fmt = (' ' * STDINDENT) + ":%s: %s"
	for k in node.attributes:
	  if   k == "alt"   : buf.append(fmt % (k, node[k]))
	  elif k == "width" : buf.append(fmt % (k, node[k]))
	  elif k == "height" : buf.append(fmt % (k, node[k]))
	attr_str = (os.linesep).join(buf)
	#print "attr_str = %s" % attr_str
	self.add_text(_('.. image:: %s\n%s') % (node['uri'].split('/')[-1], attr_str))
        raise nodes.SkipNode
